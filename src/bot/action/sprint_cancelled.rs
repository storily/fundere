...
use crate::{bot::{context::{GenericResponse, GenericResponseData}, utils::time::ChronoDateTimeExt}, db::sprint::Sprint};
...
impl SprintCancelled {
	...
	pub fn new(interaction: &Interaction, sprint: &Sprint, user: &User) -> Result<Action> {
		Ok(ActionClass::SprintCancelled(Self(GenericResponse::from_interaction(
			interaction,
			GenericResponseData {
				content: Some(format!(
